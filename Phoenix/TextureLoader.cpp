...
//
int	TextureLoader::LoadTextureFromDisk(TCHAR *szFileName, glTexture *pglTexture)
{
	TCHAR szFullPath[MAX_PATH+1];										// Full Path To Picture
	TCHAR szExtension[16];												// Extenstion of Picture

	if (_tcsstr(szFileName, _T("http://")))									// If PathName Contains http:// Then...
	{
		_tcscpy(szFullPath, szFileName);									// Append The PathName To FullPath
	}
	else																// Otherwise... We Are Loading From A File
	{
		GetCurrentDirectory(MAX_PATH, szFullPath);						// Get Our Working Directory
		_tcscat(szFullPath, _T("\\"));										// Append "\" After The Working Directory
		_tcscat(szFullPath, szFileName);									// Append The PathName
	}

	ExtensionFromFilename(szFileName, szExtension);

	// if the file is a TGA then use the TGA file loader
	if (lstrcmpi(szExtension, _T("tga")) == 0)
	{
	...
	else
	{
		...
		if (lstrcmpi(szExtension, _T("bmp")) == 0)
		{
			pglTexture->TexType = txBmp;
		}
		else if ((lstrcmpi(szExtension, _T("jpg")) == 0) ||
				 (lstrcmpi(szExtension, _T("jpeg")) == 0) )
		{
			pglTexture->TexType = txJpg;
		}
		else if (lstrcmpi(szExtension, _T("gif")) == 0)
		{
			pglTexture->TexType = txGif;
		}
		else if (lstrcmpi(szExtension, _T("ico")) == 0)
		{
			pglTexture->TexType = txIco;
		}
		else if (lstrcmpi(szExtension, _T("emf")) == 0)
		{
			pglTexture->TexType = txEmf;
		}
		else if (lstrcmpi(szExtension, _T("wmf")) == 0)
		{
...
int	TextureLoader::LoadTextureFromRam(unsigned char *pData, int Length, glTexture *pglTexture, eglTexType TexType)
{
	...
	lstrcat(szTempFileName, _T("glTEMP$$.tmp"));
...
//
int	TextureLoader::LoadTextureFromResource(unsigned int ResourceName, TCHAR *pResourceType, glTexture *pglTexture, eglTexType TexType)
{
	int	rc = FALSE;
...
//
int TextureLoader::BuildTexture(TCHAR *szPathName, glTexture *pglTexture)
{
	HDC			hdcTemp;												// The DC To Hold Our Bitmap
	HBITMAP		hbmpTemp;												// Holds The Bitmap Temporarily
	IPicture	*pPicture;												// IPicture Interface
	//TCHAR		wszPath[MAX_PATH+1];									// Full Path To Picture (WCHAR)
	...
	HRESULT hr = OleLoadPicturePath(szPathName,							// Path
									NULL,								// punkCaller
									0,									// Reserved
...
// Load a TGA file
//
int TextureLoader::LoadTGAFromDisk(TCHAR *pszFileName, glTexture *pglTexture)
{
	...
	fTGA = _tfopen(pszFileName, _T("rb"));									// Open file for reading
...
//
int TextureLoader::LoadJPG_GIFResource(TCHAR *pResourceName, TCHAR *pResourceType, glTexture *pglTexture)
{
	HDC			hdcTemp;												// The DC To Hold Our Bitmap
	HBITMAP		hbmpTemp;												// Holds The Bitmap Temporarily
	IPicture	*pPicture;												// IPicture Interface
...
int	TextureLoader::LoadTGAResource(TCHAR *pResourceName, TCHAR *pResourceType, glTexture *pglTexture)
{
	// from resources
	HRSRC	hRes;
	hRes = FindResource(NULL, pResourceName, pResourceType);
...
// extract the extension from the specified filename
//
void TextureLoader::ExtensionFromFilename(TCHAR *szFileName, TCHAR *szExtension)
{
